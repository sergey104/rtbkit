    ...
    void setConfig()
    {
        ...
        if(!cfgfile.empty()) {
            try {
                ...
                std::string cfg("");
                std::remove_copy(std::istreambuf_iterator<char>(cf), std::istreambuf_iterator<char>(), std::back_inserter(cfg), '\n');
                Json::Value jcfg = Json::parse(cfg);
...
int main(int argc, char** argv)
{
    ...
    RTBKIT::FixedPriceBiddingAgent agent(serviceProxies, "fixed-price-agent-v2", cfgfile);
