...
class Server:
	...
	def sendClickResponse(self, data, price):
		...
		clickRequest = "http://nurl.5kszypekn4.eu-west-1.elasticbeanstalk.com/?action=event&user=test&type=CLICK&auctionId=" + jsonResponse["auctionId"] + "&bidRequestId=" + jsonResponse["auctionId"] + "&impId=" + jsonResponse["impId"] + "&winPrice=" + str(jsonResponse["winPrice"])
		
		print "send click: ", clickRequest
...
#			resp = conn.getresponse()
			resp = requests.get(clickRequest)
			print "Click resp: ", resp.status_code
#			clickdata = resp.read()
#			if clickdata:
...
#			conn.close()
	...
	def sendImpressionResponse(self, data, price):
		...
		impRequest = "http://nurl.5kszypekn4.eu-west-1.elasticbeanstalk.com/?action=event&user=test&type=IMPRESSION&auctionId=" + jsonResponse["auctionId"] + "&bidRequestId=" + jsonResponse["auctionId"] + "&impId=" + jsonResponse["impId"] + "&winPrice=" + str(jsonResponse["winPrice"])
		print "send impression: ", impRequest
...
#			resp = conn.getresponse()
			resp = requests.get(impRequest)
			print "Imp resp: ", resp.status_code
#			impdata = resp.read()
#			if impdata:
#				print "data: ", impdata
#			conn.close()
			if resp.status_code == 200 and self.clickNumber < self.maxClicks:
	...
	def sendWinResponse(self, data):
		...
		price = price - delta
		jsonResponse["winPrice"] = price
		winRequest = "http://nurl.5kszypekn4.eu-west-1.elasticbeanstalk.com/?action=nurl&user=test&auctionId=" + jsonResponse["auctionId"] + "&bidRequestId=" + jsonResponse["auctionId"] + "&impId=" + jsonResponse["impId"] + "&winPrice=" + str(jsonResponse["winPrice"])
		
		print "send win: ", winRequest
...
#			resp = conn.getresponse()
			resp = requests.get(winRequest)
		except:
			print "Connection error"
			return
            
		print "Win resp: " , resp.status_code
#		windata = resp.read()
#		conn.close()
#		if windata:
#			print "windata: ", windata
		if resp.status_code == 200 and self.impNumber < self.maxImpressions:
