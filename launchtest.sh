...
tmux new-session -d -s rtb './build/x86_64/bin/launcher --node localhost --script ./launchtest.sh --launch rtbkit/sample.start.json'
...
tmux new-window -d -t rtb:7 -n 'post-auction' 'tail -F ./logs/post-auction.log'
tmux new-window -d -t rtb:8 -n 'fixed-price-agent' 'tail -F ./logs/fixed-price-agent.log'
if [[ "$*" != *--quiet* ]]
then
    tmux attach -t rtb
fi
